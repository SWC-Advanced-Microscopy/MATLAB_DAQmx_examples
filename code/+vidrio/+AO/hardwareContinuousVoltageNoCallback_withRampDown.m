function varargout = hardwareContinuousVoltageNoCallback(devName)
    ...
    %
    % Inputs
    % devName - [optional] the device ID as specified in MAX. 'Dev1' by default.
    %
    ...
    if nargout==0
        %Define a cleanup function only if user requests no outputs. This is for debugging and dev
    ...
    waveform=sin(linspace(-pi,pi, sampleRate/10))'*5; % Build one cycle of a sine wave to play through the AO line. NOTE: column vector
    ...
    try
        ...
        input(sprintf('\nPress return to ramp down amplitude\n'))

        for amp = 0.75:-0.1:0
            hTask.writeAnalogData(waveform*amp, 5)
        end
